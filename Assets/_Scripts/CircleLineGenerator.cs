...
public class CircleLineGenerator : MonoBehaviour
{
    ...
    private int calculatePostBreakAngle(GameObject finalStation){ //Probably should be moved to other region
        ...
        Debug.Log(finalStationToBreakDirection);
    ...
    private void OnTouchEnded(Vector2 touchPosition)
    {
        ...
        Color glowColor = colors[currentColorIndex]; //Code repetition
        ...
        colorChainManager.GenerateChainExtensions(colorNames[currentColorIndex], glowColor, colors[currentColorIndex]);
    ...
    private void CreateTwoPartLine()
    {
        ...
        Color glowColor = colors[currentColorIndex];
        //https://forum.unity.com/threads/setting-material-emission-intensity-in-script-to-match-ui-value.661624/
        //Some random dude found out how to set emission intensity in unity
        ...
        SetColorOfLinePart(current2PartLine.transform.Find("FirstHalf"), glowColor, originalColor);
        SetColorOfLinePart(current2PartLine.transform.Find("SecondHalf"), glowColor, originalColor);
        SetColorOfLinePart(current2PartLine.transform.Find("BreakCircle"), glowColor, originalColor);
    ...
    private void SetColorOfLinePart(Transform line, Color glowColor,Color originalColor)
    {
        line.GetComponent<SpriteRenderer>().color = originalColor;

        if(!lineGlow)
            line.GetComponent<SpriteRenderer>().material = Resources.Load<Material>("Materials/Def Unlit");
        else{
